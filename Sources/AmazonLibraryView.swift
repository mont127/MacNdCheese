...
struct AmazonGameCard: View {
    ...
    private var downloadState: AmazonDownloadState? {
        guard let amazonId = game.amazonId,
              let state = backend.amazonDownloads[amazonId],
    ...
    private func startInstall() {
        guard let prefix = backend.activePrefix, let amazonId = game.amazonId else { return }
        Task {
            _ = await backend.amazonInstallGame(prefix: prefix, amazonId: amazonId)
            await backend.refreshAmazonDownloads()
        }
    }

    private func pauseInstall() {
        guard let amazonId = game.amazonId else { return }
        Task {
            await backend.amazonPauseInstall(amazonId: amazonId)
            await backend.refreshAmazonDownloads()
        }
    }
    ...
    private func resumeInstall() {
        guard let amazonId = game.amazonId else { return }
        Task {
            await backend.amazonResumeInstall(amazonId: amazonId)
            await backend.refreshAmazonDownloads()
        }
    }
    ...
    private func cancelInstall() {
        guard let amazonId = game.amazonId else { return }
        Task {
            await backend.amazonCancelInstall(amazonId: amazonId)
            await backend.refreshAmazonDownloads()
        }
    }
